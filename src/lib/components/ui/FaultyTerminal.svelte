<script lang="ts">
  ...
  void main() {
      ...
      vec3 coral = vec3(1.0, 0.549, 0.478);  // #FF8C7A
      vec3 sand  = vec3(0.839, 0.706, 0.573); // #D6B492
      vec3 pink  = vec3(1.0, 0.518, 0.612);  // #FF849C

      // Blend colors across the screen (Left to Right)
      vec3 gradient = mix(
          mix(sand, coral, clamp(vUv.x * 2.0, 0.0, 1.0)),
          pink,
          clamp((vUv.x - 0.5) * 2.0, 0.0, 1.0)
      );
      ...
      // --- NEW: FIX THE GREY BACKGROUND BUG ---
      // Determine brightness of the pixel. If it's 0 (empty space), alpha becomes 0 (transparent).
      float pixelIntensity = max(col.r, max(col.g, col.b));
      float alpha = smoothstep(0.0, 0.15, pixelIntensity);

      gl_FragColor = vec4(col, alpha);
  }
  `;

    // References and variables (replacing React refs)
    ...
    // Svelte 5 Computed states (replacing React useMemos)
    let tintVec = $derived(hexToRgb(tint));
    let ditherValue = $derived(
    ...
    // Reactive updates for Uniforms (replacing React render dependencies)
    $effect(() => {
        if (program) {
