...
pub async fn add_watched_project(pool: &Pool, name: String, folder_path: String) -> Result<i32, AppError> {
...
pub async fn get_file_hashes(pool: &Pool, project_id: i32) -> Result<HashMap<String, String>, AppError> {
    let hashes = pool.get().await?.interact(move |conn| {
        let mut stmt = conn
